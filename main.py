...
def format_str_to_re(format_str : str):
    def to_named_group(matchobj : re.Match):
...
def requirements_satisfied(stat):
...
def get_date_from_match(matchobj : re.Match):
    ...
    try:
        ...
        exit(1)
    return datetime.strptime("[%d/%b/%Y:%H:%M:%S %z]", date_str)
...
def get_date_from_header(header : str, start : bool):
    ...
    return datetime.strptime(date_format_str, match[1])
...
def parse_dates(from_date : str, to_date : str, header : str):
    ...
    from_date = datetime.strptime(date_str, from_date) if from_date is not None else get_date_from_header(header, 1)
    to_date = datetime.strptime(date_str, to_date) if to_date is not None else get_date_from_header(header, 0)
...
def main(logfile, from_date, to_date):
    ...
    with open(logfile, 'r') as file:
        ...
        statistics_list = [stat(from_date, to_date) for stat in statistics_class_list if requirements_satisfied(stat)]
